...
\begin{algorithm}[H]
	...
	\While{running}{
		query scrapeTargets $S$ from control plane \;
		\ForEach {$s \in \mathcal S $}
		{
			query instances $I$ for $s$ from control plane \;
			get all \textit{pending} $\wedge$ \textit{terminating} instances $i_{p}, i_{t} \in I$ \;
			
		}
		set pending, marked as terminating to progress
